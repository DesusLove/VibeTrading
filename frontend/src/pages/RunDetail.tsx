  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="border-b border-border-hairline">
        ...
        {/* Metrics strip */}
        {run.metrics && (
          renderMetricsStrip(run.metrics as Record<string, number>)
        )}

        {/* Tab bar + download buttons */}
        <div className="flex items-center px-4 border-b border-border-hairline">
          <div className="tab-bar flex-1 border-0">
            {TABS.filter(t => !t.hidden).map(({ id, label, icon: Icon }) => (
              <button
                key={id}
                onClick={() => setTab(id)}
                className={cn("tab flex items-center gap-1.5", tab === id && "tab-active")}
              >
...
const METRIC_KEYS = ["total_return", "sharpe", "max_drawdown", "win_rate", "volatility", "trade_count"];

function renderMetricsStrip(metrics: Record<string, number>) {
  const items = METRIC_KEYS.map((key) => {
    const v = metrics[key];
    if (v === undefined || v === null) return null;
    let val: string;
    if (key === "total_return" || key === "max_drawdown" || key === "win_rate" || key === "volatility") {
      val = (v * 100).toFixed(2) + "%";
    } else if (key === "trade_count") {
      val = v.toFixed(0);
    } else {
      val = v.toFixed(3);
    }
